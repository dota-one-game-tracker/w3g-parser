...
static inline void readCompressedActions(::std::ifstream &replayStream, baje::ReplayHeader &header) {
	...
	for (size_t i = 0; i < header.numberOfCompressedDataBlocks; ++i) {
		...
		do {
		    ...
		    // If input consumed but not ended, we might need more input (shouldn't happen if n is correct)
		    if (r != Z_STREAM_END && strm.avail_in == 0) {
			// Some formats rely on Z_SYNC_FLUSH and don't end with STREAM_END; accept exact outSize
			break;
		    }
